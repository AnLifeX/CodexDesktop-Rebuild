...
- **会话管理兼容**：优先使用上游原生会话删除能力，仅为缺少归档删除的旧版补齐入口，同时修复旧本地会话被隐藏的问题。
...
- 当前同步版本直接使用上游原生删除入口，不重复注入自定义按钮；归档管理支持单条、按项目和全部永久删除。
- 对仍缺少归档删除能力的旧版上游，补丁会补齐单条永久删除入口。
- 删除前由对应界面二次确认，确认后调用官方 `thread/delete` 协议；此操作不可恢复。
- 修正切换登录方式、提供商或本地主机标识后，旧的本地会话可能被错误隐藏的问题；明确配置的远程主机和远程项目仍按原逻辑处理。

### Windows 兼容性

- Squirrel 构建禁用仅适用于 MSIX 包身份的 App Contained Core，避免启动时出现“该进程没有程序包标识符”。
...
Windows 更新源始终保留最新完整包，以及累计大小仍小于该完整包的最新连续残差链；残差包数量没有固定上限。`delta-chain.json` 记录每个残差包准确的起始版本和目标版本。
...
同一官方版本需要紧急热修时，可手动运行 `Build & Release Windows (Manual)`，将 `rebuild_revision` 设为正整数（首次热修填 `1`）。项目使用 `-r1`、`-r2` 作为重构修订后缀，例如 `26.915.31945-r1`；这同时生成对应的 `codex-win-26.915.31945-r1` 标签和独立更新包，不覆盖原版本。
