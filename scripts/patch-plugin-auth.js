...
function patchPluginMainSource(source) {
  ...
  // rebuild integrity check, which treats that supported state as corruption.
  if (source.includes("CodexRebuildBundledPluginIntegrity")) {
    const gates = [...source.matchAll(/&&([$\w]+\.bundledPlugin\.name)!==`browser`\/\* CodexRebuildBundledPluginIntegrity \*\//g)];
    if (gates.length !== 1) throw new Error("legacy Browser integrity gate is malformed");
    const skill = `${gates[0][1]}===\`browser\`?\`control-in-app-browser\`:\`visualize\``;
    if (source.split(skill).length !== 3) throw new Error("legacy Browser integrity paths are malformed");
    source = source.replace(gates[0][0], "").replaceAll(skill, "`visualize`");
  }
